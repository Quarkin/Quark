...
public class SecondaryDisplayLauncher extends FragmentActivity implements AppPickedCallback,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pinnedAppGridView.setOnItemLongClickListener((adapterView, view, position, id) -> {
            final AppEntry entry = mPinnedAppListAdapter.getItem(position);
            new AlertDialog.Builder(this)
                    .setTitle(entry.getLabel())
                    .setMessage(R.string.remove_pinned_app)
                    .setPositiveButton(android.R.string.ok, (dialog, which) -> {
                        removePinnedApp(entry);
                    })
    ...
    private void removePinnedApp(AppEntry appEntry) {
        final SharedPreferences sp = getSharedPreferences(PINNED_APPS_KEY, 0);
        Set<String> pinnedApps = sp.getStringSet(PINNED_APPS_KEY, null);
        if (pinnedApps != null) {
            pinnedApps = new HashSet<>(pinnedApps);
            pinnedApps.remove(appEntry.getComponentName().flattenToString());
            final SharedPreferences.Editor editor = sp.edit();
            editor.putStringSet(PINNED_APPS_KEY, pinnedApps);
            editor.apply();
        }
